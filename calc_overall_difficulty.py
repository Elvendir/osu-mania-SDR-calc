...
def accuracy_from_kps_and_player_level(x, a):
...
def calc_overall_difficulty(individual_difficulty):
    ...
    player_level_95 = 0
    b = True
    zeros = np.zeros(len(individual_difficulty))
    accuracy_VS_player_level = []
    while accuracy_at_player_level < 0.995:
        delta_diff = np.maximum(1 + (individual_difficulty - player_level), zeros)
        accuracy_at_player_level = np.mean(accuracy_from_kps_and_player_level(delta_diff, player_level))
