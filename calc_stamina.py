...
def calc_stamina(map, nb_columns):
    ...
    for i in range(1, len(map)):
        ...
        if i_columns[i - 1][column] == -1:
            ...
            (current_s, current_s_max) = dif_eq(np.array(kps_columns)[:, column], t, s_local, np.array(i_columns)[:, column], i)
    ...
    return (
    1 + np.array(s_local) + np.sqrt(nb_columns - 1) * np.array(s_global) / 4, kps_columns, i_columns, rho, s_local,
    s_local_max, s_global, s_global_max)
